"""Z.ai (Zhipu AI) adapter — GLM model family.

Uses the OpenAI-compatible ``/api/paas/v4/`` endpoint so
we can reuse the ``openai`` SDK directly.

Sign up: https://z.ai/model-api
API key: https://z.ai/model-api  (dashboard → API Keys)
...
class ZaiAdapter(BaseLLMAdapter):
    ...
    def _wrap_error(self, exc: Exception) -> LLMError:
        return openai_wrap_error(exc, "zai")
